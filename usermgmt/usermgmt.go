 ...
 * @Author: thonsun
 * @Date: 2020-10-05 16:20:35
 * @Last Modified: thonsun, 2020-10-05  16:20:35
 */
...
var (
	store = sessions.NewCookieStore([]byte("asec-app-gateway"))
)
...
func Login(w http.ResponseWriter, r *http.Request, param map[string]interface{}) (*models.AuthUser, error) {
	...
	utils.DebugPrintln("tmphashpwd:",tmpHashpwd)
	utils.DebugPrintln("db:",appUser.HashPwd)
	if tmpHashpwd == appUser.HashPwd {
		...
		session.Options = &sessions.Options{Path: "/asec-admin/", MaxAge: 86400 * 7}
		session.Save(r, w)
		return authUser, nil
	} else {
...
func Logout(w http.ResponseWriter, r *http.Request) error {
	...
	session.Options = &sessions.Options{Path: "/asec-admin/", MaxAge: 0}
	session.Save(r, w)
	return nil
}
