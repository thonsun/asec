...
install_dir="/usr/local/asec"
#pg_version=`psql -V | awk -F " " '{print $3}'`

printf "Installing Asec Application Gateway... \n"
printf "Requirements:\n"
...
full_service_path=/lib/systemd/system/asec.service
if [ $os == "centos" ] || [ $os == "rhel" ]; then
    full_service_path=/usr/lib/systemd/system/asec.service
elif [ $os == "debian" ]; then
    full_service_path=/lib/systemd/system/asec.service
fi
...
if [ $option == 1 ]; then
    old_pg=`cat ./asec.service |grep postgres | awk -F "=" '{print $2}'`
    new_pg=`systemctl list-unit-files | grep postgres | head -1 | awk -F " " '{print $1}'`
    if [ -z "$new_pg" ]; then
        # No PostgreSQL, delete After=postgresql.service
        sed -i '/postgres/d' ./asec.service
    else
        # Exist PostgreSQL
        sed -i "s/$old_pg/$new_pg/" ./asec.service
    fi    
    \cp -f ./asec.service ${full_service_path}
    ...
    \cp -f ./asec.service ./asec-replica.service
    sed -i '/postgres/d' ./asec-replica.service
    \cp -f ./asec-replica.service ${full_service_path}
